...
It has a variable called `labelset` - which contains the list of labels for the current for loop. As a quick reminder, in every iteration - if some condition is satisfied, for loop executes the body of the loop. Now, after for loop executes the body of the loop - it looks at its completion record.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTU4ODU5MTA5LC02NjI2MzE3ODMsLTY2Mj
