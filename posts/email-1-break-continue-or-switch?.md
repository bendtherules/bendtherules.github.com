...
It has a variable called `labelset` - which contains the list of labels for the current for loop. Quick reminder to for loops - in every iteration, if some condition is satisfied - it executes the body . Now, after for loop executes the body of the loop - it looks at its completion record.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwNzMzMzU3MTYsLTY2MjYzMTc4MywtNj
