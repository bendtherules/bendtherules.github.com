...
⭐️For ex - when you use a return statement deep within a function, all inner constructs stop their algorithm immediately and forward the same completion record to their parent. This happens till it hits the parent function. Now, a function knows how to "handle" a return completion - so instead of forwarding it again, it converts that to a normal completion with the same value.

How does `break foo;` work? 

<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE1ODk1ODQxMzMsMTEzOTQwODQ5MCw0Nz
