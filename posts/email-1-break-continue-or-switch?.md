...
Now let's talk about the algo. How does for loop (or other iteration statements) handle continue and break?  
Quick reminder to for loops - in every iteration, if some condition is satisfied then it executes the body and then increments some variable. It has a variable called `labelset` - which contains the list of labels for the current for loop. Now, after for loop executes the body of the loop - it looks at its completion record.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjA3MDE4MTYyMCwtNjYyNjMxNzgzLC02Nj
