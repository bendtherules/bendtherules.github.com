...
Also, if you are thinking that "*ehh, I will just set fn.name to whatever i want* after creating the function", that is not possible. `.name` property descriptor has `writeable: false`. So, if you try to set `.name` to some value, it will simply NOT affect the `name` (non-strict) or throw `TypeError` is strict mode.
...
BnZm1cbiIsImhpc3RvcnkiOlstMzc2MjA3MjIzLC0yMDg1ODgx
