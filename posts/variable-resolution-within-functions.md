...
A function has 2 distinct phases - function creation and function call.  Function is created when you define it (with hoisting) and that scope where it is created can be called as lexical scope or creation scope (IMO, lexical scope is a overused term, so we'll stick with c). can be created/defined in a scope (say, creation scope), stored and called later in a different scope (caller scope). 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwNzQ3MDgyMjIsLTE2NjIzMTY5NTYsLT
